...
library(magrittr)
library(dplyr)
realtor=read.csv('~/Documents/NYC/Scrapy/Realtor/Realtor/Realtor_listing_Nov_clean.csv',stringsAsFactors = FALSE)
a=summary(realtor)
...
days.median<-median(realtor$days_on_market,na.rm=TRUE)
realtor$days_on_market<-ifelse(is.na(realtor$days_on_market),days.median,realtor$days_on_market)

realtor$area_ft[is.na(realtor$area_ft)] <- ave(realtor$area_ft, realtor$structure, FUN=function(x)mean(x,na.rm = T))[is.na(realtor$area_ft)] 
realtor$lot_ft[is.na(realtor$lot_ft)] <- ave(realtor$lot_ft, realtor$property_type, FUN=function(x)mean(x,na.rm = T))[is.na(realtor$lot_ft)] 
